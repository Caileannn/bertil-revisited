...
public class Chair : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if(_BrainType == BrainType.Walk || _BrainType == BrainType.Balance){
        ...
        } else if(_BrainType == BrainType.Sit){
            if(_randomiseYRotation){
                bpSeat.rotation = Quaternion.Euler(0, Random.Range(0, 360), 0);
            }
        }
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        if(_BrainType == BrainType.Getup || _BrainType == BrainType.Balance){
    ...
    void FixedUpdate()
    {
        ...
        if(_BrainType == BrainType.Walk){
            ...
            var distanceProgress = _previousDistanceFromTarget - currentDistanceFromTarget;

            // // if the distance reward is 0, set it to a small value to avoid division by 0
            // if(distanceProgress > 0){
            //     score = Mathf.Min(distanceProgress * 0.1f, 1.0f);
            //     AddReward(score);
            // }

            float progress = (_InitialDistanceFromTarget - currentDistanceFromTarget) / _InitialDistanceFromTarget;

            if (progress > 0) 
            {
                AddReward(Mathf.Pow(progress * 0.2f, 2)); // 0.04 - 0.01 = 0.03 ~ 100+ 0.01 * 5000 = 50 mean reward
            }
            ...
            if(currentDistanceFromTarget < _ClosestDistanceFromTarget){
                ...
                score = Mathf.Min(distanceProgress * 10.0f, 1.0f);
                //AddReward(Mathf.Pow(score, 2.0f));
                //Debug.Log($"New score!");
            }

            // How many steps have past since their was progress
            ...
            var height = CheckRaycastHeight();
            var clampedHeight = Mathf.Pow(1 - height, 2);
            Debug.Log($"Clamped Height: {clampedHeight}");
            AddReward(clampedHeight);
        } else if(_BrainType == BrainType.Balance){
            Vector2 deltaAngle = GetAngleDeltaXZ();
            float angleDeltaReward = Mathf.Pow(deltaAngle.x * deltaAngle.y, 2);
            AddReward(angleDeltaReward);
            //Debug.Log($"balance reward: {angleDeltaReward}");
        }
    }
    ...
    private float CheckRaycastHeight(){
        ...
        if(Physics.Raycast(bpSeat.transform.position, transform.TransformDirection(Vector3.down), out hit, Mathf.Infinity)){
            Debug.DrawRay(bpSeat.transform.position, transform.TransformDirection(Vector3.down) * hit.distance, Color.green);
